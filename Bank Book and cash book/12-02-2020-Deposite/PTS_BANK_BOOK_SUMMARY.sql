...
ALTER PROCEDURE [dbo].[PTS_BANK_BOOK_SUMMARY]
...
,(CASE when T3.Segment_0 <> '' then T3.Segment_0 
       ...
       (case when T2.TransType = 46 then T4.CardCode 
            ...
            when t2.transtype=25  then chh.CardCode End)
       else T3.AcctCode + '' 
        End)'BP/GL Code'
,(CASE when T3.Segment_0 <> '' then  T3.Segment_1 + ' ' + T3.AcctName 
       ...
       (case when T2.TransType = 46 then  T4.CardName 
            ...
            when t2.transtype=25  then chh.cardname End)
       else T3.AcctCode + '' 
...
left outer join OVPM T4 on T4.DocNum =(CASE when  T1.TransType = 46 then T1.BaseRef  else '' end) and T1.RefDate = T4.TaxDate
left outer join ORCT T6 on T6.DocNum =(CASE when  T1.TransType = 24 then T1.BaseRef  else '' end) and T1.RefDate = T6.TaxDate
left outer join ODPS dps on dps.transabs=t1.transid
left outer join OCHH CHH on cast(chh.checknum as varchar)=cast(t1.ref3line as varchar)
where 
